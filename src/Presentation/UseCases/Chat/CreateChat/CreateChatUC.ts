...
export class CreateChatUC {
  ...
  async execute(data: ICreateChatRequestDTO): Promise<IReturnAdapter> {
    try {
      ...
      let NewChat: Chat;
      if (userData.chatID !== "") {
        const findChat = await this.chatRepository.readMessages(
          userData.chatID
        );
        if (findChat.val === false) {
          throw new Error(findChat.erro);
        }
        NewChat = new Chat(uid, findChat.data, userData.chatID);
      } else {
        ...
        const update = await this.usersFireStoreRepository.update(
          data.uid,
          "chatID",
          NewChat.chatid
        );
